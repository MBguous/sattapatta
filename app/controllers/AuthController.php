...
class AuthController extends \BaseController implements UserInterface
{
	...
	public function postLogin(){
			...
			true))
		{
			...
			return Redirect::route('items.browse', array($username))
	...
	public function getSocialLogin($auth=NULL) {
		...
		if(is_null($oauthUser)) {
			...
			return Redirect::route('items.browse', array($user->username))
			...
			return Redirect::route('items.browse', array($oauthUser->username))
	...
	public function getTwitterLogin($auth=NULL){
		...
		return Redirect::intended('items.browse')->withMessage('Welcome '.$profile->firstName);
	}
	...
	public function getGoogleLogin($auth=NULL){
		...
		return Redirect::intended('items.browse')->withMessage('Welcome '.$profile->displayName)->with('profile', $profile);
	}
	...
	public function getFacebookLogin($auth=NULL){
		...
		return Redirect::intended('items.browse')->withMessage('Welcome '.$user->displayName);
	}
